...
namespace OutdoorWeiXin.Web._Code
{
    ...
    public partial class CustomMessageHandler : MessageHandler<CustomMessageContext>
    {
        ...
        public override IResponseMessageBase OnTextRequest(RequestMessageText requestMessage)
        {
            ...
            if (requestMessage.Content == "菜单")
            {
                responseMessage.Content = "<a href=\"" + weixinURL + "Menu/\">点击这里</a>进行客户端菜单自定义（地址：" + weixinURL + "Menu/）。";
            }
            if (requestMessage.Content == "二维码")
            {
                responseMessage.Content = "<a href=\"" + weixinURL + "Home/\">点击这里</a>进行RadioBuy公众平台二维码展示（地址：" + weixinURL + "Home/）。";
            }
            if (requestMessage.Content == "退出")
            {
                responseMessage.Content = "<a href=\"" + weixinURL + "LoginOut/\">点击这里</a>退出RadioBuy微信公众平台（地址：" + weixinURL + "LoginOut/）。";
            }
            ...
            //{
            //    /* 
            //    * 这是一个特殊的过程，此请求通常来自于微微嗨（http://www.weiweihi.com）的“RadioBuy网络小助手”应用请求（http://www.weiweihi.com/User/App/Detail/1），
            //    * 用于演示微微嗨应用商店的处理过程，由于微微嗨的应用内部可以单独设置对话过期时间，所以这里通常不需要考虑对话状态，只要做最简单的响应。
            //    */
            //    if (requestMessage.Content == "测试")
            //    {
            //        //进入APP测试
            //        responseMessage.Content = "您已经进入【RadioBuy网络小助手】的测试程序，请发送任意信息进行测试。发送文字【退出】退出测试对话。10分钟内无任何交互将自动退出应用对话状态。";
            //    }
            //    else
            //    {
            //        //退出APP测试
            //        responseMessage.Content = "您已经退出【RadioBuy网络小助手】的测试程序。";
            //    }
            //}
            //else
            //{
        ...
        //    responseMessage.Music.MusicUrl = "http://weixin.radiobuy.cn/Content/music1.mp3";
